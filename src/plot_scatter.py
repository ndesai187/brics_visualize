...
def plotSimpleScatterGraph(years_x_axis, country_list, country_data_y_axis, y_axis_label_lst, output_png):
    ...
    y_min_lst=list()
    y_max_lst=list()
    for list_y in country_data_y_axis:
        y_min_lst.append(min(list_y))
        y_max_lst.append(max(list_y))
    ...
    plt.ylim(min(y_min_lst),max(y_max_lst))
...
def plotScatterMultiAxisY(years_axis, country_list, female_data_y_axis, male_data_y_axis, ppp_df, y_axis_label_1, y_axis_label_2):
    ...
    # find min and max for Y axis left
    y_min_lst = list()
    y_max_lst = list()
    for list_y in female_data_y_axis+male_data_y_axis:
        y_min_lst.append(min(list_y))
        y_max_lst.append(max(list_y))
    # print(y_min_lst, y_max_lst)

    # find min and max for Y axis right
    y_min_lst_right = list()
    y_max_lst_right = list()
    for list_y in asList_PPP:
        y_min_lst_right.append(min(list_y))
        y_max_lst_right.append(max(list_y))
    # print(y_min_lst, y_max_lst)
    ...
    for i in range(0, len(country_list)):
        ...
        ax1.set_ylim(min(y_min_lst), max(y_max_lst))
        ax2.set_ylim(min(y_min_lst_right), max(y_max_lst_right))
