...
def genderFilteredByIndicator(infile, yearColumnFrom, yearColumnTo, indicator, step_data_points):
    ...
    gender_stats_byCol_list = gender_stats_byRow_df.loc[:, yearColumnFrom: yearColumnTo : step_data_points].values.tolist()
...
def trigger_scatter_plot(infile, indicator, step_data_points):
    ...
    print(gender_stats_byCol_list)
    tmp = genderFilteredByIndicator(infile,str(1980),str(2017),indicator,step_data_points)

    if(gender_stats_byCol_list == tmp):
        print(True)
    else:
        print(False)
...
def trigger_unemloyement_vs_ppp(inFile_ppp, inFile_gender, indicator):
    ...
    print(gender_stats_byCol_list)
    # # call plot function to create graph
    plot_scatter.plot_3x_graph(list(range(2000, 2018)), country_list_name, gender_stats_byCol_list, dataset_df)
...
def visualize():
    ...
    # read ppp excel
    unemp_indicator = "SL.UEM.TOTL.FE.ZS"
    ppp_stats_inFile = "../resources/CustomQuery-8_9_2018.xls"
    trigger_unemloyement_vs_ppp(ppp_stats_inFile, gender_stats_inFile, unemp_indicator)
